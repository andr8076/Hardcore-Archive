...
grep -Fq 'Modern ffprobe could not inspect' "$TOOLS/prove-p530.sh"
grep -Fq 'Modern ffprobe could not read the legacy HEVC duration' "$TOOLS/prove-p530.sh"
grep -Fq 'Modern FFmpeg could not fully decode' "$TOOLS/prove-p530.sh"
grep -Fq 'CODEC=$("$MODERN_FFPROBE"' "$TOOLS/prove-p530.sh"

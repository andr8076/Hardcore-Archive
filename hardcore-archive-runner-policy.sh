...
if $CONFIG_ENABLED; then
    ...
    if [[ ${HARDCORE_ARCHIVE_CALIBRATION_POLICY_INHERITED:-0} != 1 ]]; then
        CONFIG_VIDEO_CUDA_DEVICE=$(config_value VIDEO_CUDA_DEVICE "$CONFIG_FILE" || true)
    fi
...
export HARDCORE_ARCHIVE_VIDEO_CUDA_DEVICE=${CONFIG_VIDEO_CUDA_DEVICE:-${HARDCORE_ARCHIVE_VIDEO_CUDA_DEVICE:-0}}
if [[ ! $HARDCORE_ARCHIVE_VIDEO_CUDA_DEVICE =~ ^(0|[1-9][0-9]{0,2})$ ]]; then
    printf 'Error: VIDEO_CUDA_DEVICE must be a GPU index from 0 to 999.\n' >&2
    exit 2
fi

...
- The expected VMAF frame population is counted from candidate **display intervals** that overlap the requested half-open interval `[start, end)`, using only a **1 microsecond comparison epsilon** for floating-point representation. This matches FFmpeg input-side accurate seeking, which can score the frame already displayed at a fractional start timestamp even when that frame's PTS is slightly earlier. The wider 50 ms timeline tolerance is deliberately not used to invent boundary frames.
...
2. The number of VMAF records exactly equals the number of candidate display intervals overlapping `[start, end)`.
